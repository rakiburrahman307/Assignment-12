...
const Detail = () => {
  ...
  const { data: reviewsConfirm = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/all_meals/${_id}/reviewConfirm/${customerEmail}`
      );
      return res.data;
    },
  });
  ...
  const handleComment = (e) => {
    ...
    if (reviewsConfirm?.reviewExists === true) {
      ...
      axiosSecure
        .post(`/all_meals_review/${_id}`, comment)
        .then(() => {
          axiosSecure
            .post(`/reviewsCollections`, comment)
            .then(() => {
  ...
  return (
    <div data-aos='fade-up'>
      <div className='card card-side bg-base-100 shadow-xl'>
        <img
          className={`w-96 object-fill mx-10 transition-opacity duration-500 ${
        ...
        <div className='card-body'>
          <h2 className='card-title'>
            <span className='text-xl font-semibold'>Type:</span> {mealType}
          </h2>
          <h4 className='text-xl font-semibold tracking-tight text-gray-900 dark:text-white'>
            <span className='text-xl font-semibold'>Distributor:</span>{" "}
            {adminName}
          </h4>
          ...
          <div className='flex items-center'>
            <button
              onClick={handleLike}
              className='bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800 ms-3 flex gap-2'
            >
              {liked ? <BiSolidDislike /> : <BiSolidLike />} {likeCount}
            </button>
          </div>
          <p className='text-lg'>
            <span className='text-xl font-semibold'>PostTime:</span> {postTime}
          </p>
